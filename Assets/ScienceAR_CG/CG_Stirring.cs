...
public class CG_Stirring : MonoBehaviour
{
    ...
    void ResetVariables()
    {
        ...
        Step1isOK = false;
        Step2isOK = false;
        Step1_Stirring = false;
        Step2_Stirring = false;
        Step3_Stirring = false;
        isTouch_DishBowlThing = false;
        isLitmusPaper = false;
        isinbeaker = false;
    }
    ...
    void Update()
    {
        ...
        if (CG_Hint_Manager.Currentstep == 1 && !Stirring1_NE && Step1isOK)
        {
            if (PlayerPrefs.GetInt("ReviewMistakes", 0) != 1)   //實驗環節
            {
                CG_Hint_Manager.Currentstep = 2;
            }
        }
        else if (CG_Hint_Manager.Currentstep == 2 && !Stirring2_NE && Step2isOK)
        {
            if(!CG_Beaker_Anim.LampOpen)
            {
                if (PlayerPrefs.GetInt("ReviewMistakes", 0) != 1)
                {
                    CG_Hint_Manager.Currentstep = 3;
                    Debug.Log("觸發BackToExplain, Step 3 完成");
                }
            }
        }
        else if (CG_Hint_Manager.Currentstep == 3 && CG_Beaker_Anim.step3isOK) //原本是step3_times >= 6直接跳步驟四
        {
            if (PlayerPrefs.GetInt("ReviewMistakes", 0) != 1)
            {
                CG_Hint_Manager.Currentstep = 4;
            }
        }
