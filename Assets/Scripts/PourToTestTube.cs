...
public class PourToTestTube : MonoBehaviour
{
    //本腳本放在固定試管身上。
    ...
    void Update()
    {
        ...
        if (shakeCount >= 6 && Spoon_Collision.isgotoTestTube && !Turbid) // 設1.0單位範圍有效
        {
            StartCoroutine(Observation());
        }
        SaladOil_rotationX1 = Mathf.Lerp(185f, 135f, Mathf.InverseLerp(270f, 320f, ROT_Detect.rot_SaladOil));
        SaladOil_rotationX2 = Mathf.Lerp(40f, -10f, Mathf.InverseLerp(60f, 110f, ROT_Detect.rot_SaladOil));
        H2O_rotationX1 = Mathf.Lerp(185f, 135f, Mathf.InverseLerp(270f, 320f, ROT_Detect.rot_H2O));
        H2O_rotationX2 = Mathf.Lerp(40f, -10f, Mathf.InverseLerp(60f, 110f, ROT_Detect.rot_H2O));

        /*if (ROT_Detect.rot_SaladOil >= 270 && ROT_Detect.rot_SaladOil <= 320)
        {
            SaladOilParticle1.Play();
            SaladOilPGM1.transform.localRotation = Quaternion.Euler(SaladOil_rotationX1, 0, 0);
        }
        if (ROT_Detect.rot_SaladOil >= 60 && ROT_Detect.rot_SaladOil <= 110)
        {
            SaladOilParticle2.Play();
            SaladOilPGM2.transform.localRotation = Quaternion.Euler(SaladOil_rotationX2, 0, 0);
        }
        if (ROT_Detect.rot_H2O >= 270 && ROT_Detect.rot_H2O <= 320)
        {
            H2OParticle1.Play();
            H2OPGM1.transform.localRotation = Quaternion.Euler(H2O_rotationX1, 0, 0);
        }
        if (ROT_Detect.rot_H2O >= 60 && ROT_Detect.rot_H2O <= 110)
        {
            H2OParticle2.Play();
    ...
    public void OnTriggerStay(Collider other)   //判斷溶液的倒入動畫
    {
        if (other.CompareTag("H2O"))
        {
            if (ROT_Detect.rot_H2O >= 60 && ROT_Detect.rot_H2O <= 110 || ROT_Detect.rot_H2O >= 270 && ROT_Detect.rot_H2O <= 320)
            {
                H2O_isPoured = true;
        ...
        if (other.CompareTag("SaladOil"))
        {
            if (ROT_Detect.rot_SaladOil >= 60 && ROT_Detect.rot_SaladOil <= 110 || ROT_Detect.rot_SaladOil >= 270 && ROT_Detect.rot_SaladOil <= 320)
            {
