...
public class Beaker_Anim : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Hint_Manager.Currentstep == 1 && PourToBeaker.Coconut_isPoured && !isCoconutAniPlay)   //倒入溶液的計量動畫
        {
            BeakerAnim.Play("Step1Add_1");
            isCoconutAniPlay = true;
        }
        else if (Hint_Manager.Currentstep == 1 && PourToBeaker.Alcohol_isPoured && !isAlcoholAniPlay)
        {
            BeakerAnim.Play("Step1Add_2");
            isAlcoholAniPlay = true;
        }
        if (Hint_Manager.Currentstep == 2 && PourToBeaker.NaOH_isPoured && !isNaOHAniPlay)
        {
        ...
        if (Hint_Manager.Currentstep == 3 && PourToBeaker.NaCl_isPoured && !isNaClAniPlay)
        {
            BeakerAnim.Play("Step3Add");
            isNaClAniPlay = true;
        }
        if (Stirring.step3_times >= 12 && !isstep3AniPlay)
        {
            isstep3AniPlay = true;
