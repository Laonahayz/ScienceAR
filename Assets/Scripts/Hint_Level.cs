...
public class Hint_Level : MonoBehaviour
{
    [Header("大Canvas提示文字")]
    ...
    public void StopTimer()
    {
        if (isTiming)
        {
            ...
            BehaviorLogger.Instance.LogBehavior("操作總時間：" + elapsedTime + " 秒");
        }
    }
    ...
    private void StopHintTimer()
    {
        if (hintCoroutine != null)
        {
            ...
            if (errorCount >= 25)
            {
                BehaviorLogger.Instance.LogBehavior("操作錯誤次數：" + errorCount + " 次，操作成功率低。");
            }
            else
            {
                BehaviorLogger.Instance.LogBehavior("操作錯誤次數：" + errorCount + " 次，操作成功率高。");
            }
        }
    }
    ...
    private void Step1Hints()
    {
        ...
        if (!PourToBeaker.CoconutOilTrigger && !PourToBeaker.Coconut_isPoured)
        {
        ...
        else if (Stirring.Step1_Stirring && Stirring.Stirring1_NE)
        {
    ...
    private void Step2Hints()
    {
        ...
        else if (Beaker_Anim.LampOpen && !PourToBeaker.NaOHTrigger && !PourToBeaker.NaOH_isPoured)
        {
            Canvas_Hint.SetActive(false);
        ...
        else if (PourToBeaker.NaOHTrigger && !PourToBeaker.NaOH_isPoured)
        {
            UpdateBeakerHint("旋轉倒入NaOH", false, true, false, false, PourSolutions[2], PourSolutions[2], "Hint_RotAnianim");
            //Debug.Log("沒有倒入NaOH的動作");
            StartHintTimer();
        }
        else if (PourToBeaker.NaOH_isPoured && !Stirring.Step2_Stirring)
        {
            StirringRod.SetActive(true);
            UpdateBeakerHint("長按以拖曳玻棒，並左右攪拌燒杯內溶液", false, false, true, false, null, null, "Hint_STI_RotAnianim");
            //Debug.Log("沒有攪拌動作");
        ...
        else if (Stirring.Step2_Stirring && Stirring.Stirring2_NE)
        {
            UpdateBeakerHint("攪拌次數不夠", false, false, true, false, null, null, "Hint_STI_RotAnianim");
            //Debug.Log("沒有攪拌到指定次數");
            StartHintTimer();
    ...
    private void Step3Hints()
    {
        ...
        if (!PourToBeaker.NaClTrigger && !PourToBeaker.NaCl_isPoured)
        {
            StirringRod.SetActive(false);
            Beaker_Canvas.SetActive(true);
        ...
        else if (Stirring.Step3_Stirring && Stirring.Stirring3_NE)
        {
            UpdateBeakerHint("攪拌燒杯溶液，並觀察燒杯內溶液變化", false, false, true, false, null, null, "Hint_STI_RotAnianim");
            StartHintTimer();
        }
        else if (!Stirring.Stirring3_NE && Beaker_Anim.isstep3AniPlay)
        {
    ...
    private void Step4Hints()
    {
        ...
        if (!Spoon_Collision.is_scoop_up)
        {
        ...
        else if (Spoon_Collision.is_scoop_up)
        {
    ...
    private void Step5Hints()
    {
        ...
        //以下是第五步驟的提示了
        if (!Buttons_Control.isOpenLitmusPaper)
        {
        ...
        else if (Buttons_Control.isOpenLitmusPaper && !Spoon_Collision.isTouchingLitmusPaper && !Spoon_Collision.cancloseLitmusPaper)
        {
        ...
        else if (Spoon_Collision.isTouchingLitmusPaper && !Spoon_Collision.cancloseLitmusPaper)
        {
            Canvas_Hint.SetActive(false);
        ...
        else if(Spoon_Collision.cancloseLitmusPaper)
        {
    ...
    private void Step7Hints()
    {
        ...
        else if (Spoon_Collision.is_scoop_up && !Spoon_Collision.isgotoTestTube)
        {
            //這邊的圖好像是以試管為主才對??
        ...
        else if (Spoon_Collision.isgotoTestTube && !PourToTestTube.shakeTestTube)
        {
            UpdateBeakerHint("長按試管以拖曳，並上下搖晃試管觀察變化", false, false, false, true, null, null, "Hint_ShakeAni");
            //Debug.Log("需要搖晃試管");
            StartHintTimer();
        }
        else if (PourToTestTube.shakeTestTube && PourToTestTube.shakeCount < 5)
        {
            UpdateBeakerHint("上下搖晃次數不夠", false, false, false, true, null, null, "Hint_ShakeAni");
            //Debug.Log("搖晃次數不足");
            StartHintTimer();
        }
        else if (PourToTestTube.shakeCount > 5)
        {
    ...
    {
        if (stepHints.ContainsKey(Hint_Manager.Currentstep))
        {
            stepHints[Hint_Manager.Currentstep]();
        }
